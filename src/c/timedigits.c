...
// Frees the blink animation once it has finished running; the double-rate
// blink schedules a new one every second, so each must be reaped.
static void separator_animation_stopped(Animation *animation, bool finished, void *context) {
  if (separator_animation != NULL && (Animation *) separator_animation == animation) {
...
void handle_tick(struct tm *tick_time, TimeUnits units_changed) {
  ...
  if ((units_changed & SECOND_UNIT) && !powerSaveEngaged) {
    ...
    if((global_settings.Blink == BLINK_ON) && !powerSaveEngaged) {
      ...
      // Destroy the previous animation first: at double-rate blink this runs
      // every second and the old PropertyAnimation would otherwise leak (the
      // stopped-handler below frees it once it completes).
      if (separator_animation != NULL) {
        animation_unschedule((Animation *) separator_animation);
        property_animation_destroy(separator_animation);
        separator_animation = NULL;
      }
      ...
      animation_set_handlers((Animation *) separator_animation, (AnimationHandlers) {
        .stopped = (AnimationStoppedHandler) separator_animation_stopped,
      }, NULL);
...
void timedigits_init() {
  ...
  //Date - SUN 15
  t_layer[t_date] = text_layer_create_detailed(TIMEDIGITS_DATE,
  ...
  // Launch cascade (first init only): digits slide in from the left, the
  // date from the right.
  if (!appStarted) {
